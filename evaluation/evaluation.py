...
class Evaluator:
    ...
    def evaluate_task_2(self, predictions, true_data, prediction_file_path):
        ...
        # Combined Evaluation
        true_combined_targets = [(item['TARGET'],) + tuple(item['TARGETS']) for item in true_data]
        pred_combined_targets = [(item['TARGET'],) + tuple(item['TARGETS']) for item in predictions]
